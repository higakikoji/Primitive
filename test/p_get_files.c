		...
		if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			if (sub_directory == 1) {
				if (strncmp(ffd.cFileName, P_CURRENT_DIRECTORYA, P_CURRENT_DURECTORY_LENGTH) != 0 && strncmp(ffd.cFileName, P_PARENT_DIRECTORYA, P_PARENT_DIRECTORY_LENGTH) != 0) {
					...
					int count = p_get_file_counta((LPCSTR)p_tmp_patha, search_extension, sub_directory);
					if (count < 0) {
						break;
					}
					ret += count;
		...
		if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			if (sub_directory == 1) {
				if (wcsncmp(ffd.cFileName, P_CURRENT_DIRECTORYW, P_CURRENT_DURECTORY_LENGTH) != 0 && wcsncmp(ffd.cFileName, P_PARENT_DIRECTORYW, P_PARENT_DIRECTORY_LENGTH) != 0) {
					...
					int count = p_get_file_countw((LPCWSTR)p_tmp_pathw, search_extension, sub_directory);
					if (count < 0) {
						break;
					}
					ret += count;
	...
	do {
		if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			if (sub_directory == 1) {
				if (strncmp(ffd.cFileName, P_CURRENT_DIRECTORYA, P_CURRENT_DURECTORY_LENGTH) != 0 && strncmp(ffd.cFileName, P_PARENT_DIRECTORYA, P_PARENT_DIRECTORY_LENGTH) != 0) {
					p_sprintfa((char *const)p_tmp_patha, MAX_PATH, MAX_PATH, NULL, "%s\\%s", path, ffd.cFileName);
					int count = p_get_filesa((LPCSTR)p_tmp_patha, search_extension, sub_directory, &files[ret], buffer_count);
					if (count < 0) {
						break;
					}
					ret += count;
			...
			if (search_extension == NULL) {
				p_sprintfa((char *const)p_tmp_patha, MAX_PATH, MAX_PATH, NULL, "%s\\%s", path, ffd.cFileName);
				strncpy_s(files[ret], MAX_PATH, (const char*)p_tmp_patha, MAX_PATH);
				ret++;
			} else {
				if (strncmp(search_extension, P_WILD_CARDA, P_WILD_CARD_LENGTH) == 0) {
					p_sprintfa((char *const)p_tmp_patha, MAX_PATH, MAX_PATH, NULL, "%s\\%s", path, ffd.cFileName);
					...
					if (e_len > 0) {
						if (strncmp(&e[1], search_extension, strnlen_s(search_extension, P_EXTENSION_MAX_LENGTH)) == 0) {
							p_sprintfa((char *const)p_tmp_patha, MAX_PATH, MAX_PATH, NULL, "%s\\%s", path, ffd.cFileName);
							strncpy_s(files[ret], strnlen_s(p_tmp_patha, buffer_count) + 1, (const char *)p_tmp_patha, buffer_count);
							ret++;
						}
					}
				}
	...
	do {
		if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			if (sub_directory == 1) {
				if (wcsncmp(ffd.cFileName, P_CURRENT_DIRECTORYW, P_CURRENT_DURECTORY_LENGTH) != 0 && wcsncmp(ffd.cFileName, P_PARENT_DIRECTORYW, P_PARENT_DIRECTORY_LENGTH) != 0) {
					p_sprintfw((wchar_t *const)p_tmp_pathw, MAX_PATH, MAX_PATH, NULL, L"%s\\%s", path, ffd.cFileName);
					int count = p_get_filesw((LPWSTR)p_tmp_pathw, search_extension, sub_directory, &files[ret], buffer_count);
					if (count < 0) {
						break;
					}
					ret += count;
			...
			if (search_extension == NULL) {
				p_sprintfw((wchar_t *const)p_tmp_pathw, MAX_PATH, MAX_PATH, NULL, L"%s\\%s", path, ffd.cFileName);
				wcsncpy_s(files[ret], MAX_PATH, (const wchar_t *)p_tmp_pathw, MAX_PATH);
				ret++;
			} else {
				if (wcsncmp(search_extension, P_WILD_CARDW, P_WILD_CARD_LENGTH) == 0) {
					p_sprintfw((wchar_t *const)p_tmp_pathw, MAX_PATH, MAX_PATH, NULL, L"%s\\%s", path, ffd.cFileName);
					...
					if (e_len > 0) {
						if (wcsncmp(&e[1], search_extension, wcsnlen_s(search_extension, P_EXTENSION_MAX_LENGTH)) == 0) {
							p_sprintfw((wchar_t *const)p_tmp_pathw, MAX_PATH, MAX_PATH, NULL, L"%s\\%s", path, ffd.cFileName);
							wcsncpy_s(files[ret], wcsnlen_s(p_tmp_pathw, buffer_count) + 1, (const wchar_t *)p_tmp_pathw, buffer_count);
							ret++;
						}
					}
				}
